    ...
    try:
        ...
        for _, row in fighters_df.iterrows():
            ...
            if wins > 0:
                ...
                if (sub_wins_ratio > 0.13) and (sub_att / total_fights > 0.48) and (total_fights >= 5):
                ...
                if (ground_finish_rate > 55) and (ground_landed_per_tko > 15) and (ctrl_avg > 150) and (total_fights >= 5) and (ko_tko_wins > 0):
                    ...
                    doughmaker_count += 1
                if ((leg_landed_avg > 23) or (body_landed_avg > 25)) and (leg_landed_avg + body_landed_avg > 50) and (ko_tko_wins > 2) and (total_fights >= 5):
            ...
            if (kd_received_avg < 0.25) and (ko_loss_rate < 6) and (total_fights >= 5):
                fighter_badges.append('Iron Chin')
                iron_chin_count += 1
            if (sub_att_received_avg < 0.7) and ((sub_def > 90) or (sub_att_received_avg == 0)) and (never_submitted == 1) and (total_fights >= 5):
                fighter_badges.append('Locksmith')
                locksmith_count += 1
            if (total_fights >= 5) and (total_fight_time_sec / total_fights > 150) and (sig_str_landed_per_sec > 0.22):
